...
function apagar(){
    if(confirm("voce deseja apagar?")==0){
        window.location.replace("index.html");
        return;
    }
    fetch(`http://localhost:5000/produto/apagar/${window.location.search.substring(4)}`,{
        method:"DELETE",
        headers:{
...
function carregarAtualizar() {
    //obter o id passado na barra de endreço
    var id = window.location.search.substring(4);
    //fazer uma busca para receber o produto especifico do formulario
...
function atualizar(){
    if (confirm("Voçe deseja atualizer este item?")==0){
        return;
    }
